...
### Visualization: Example with top H across samples

```{r, message=FALSE, warning=FALSE}
# Define output path
mm909_h_plot_path <- file.path(figures_path,
                              "mm909_h_plot.png")

# Prepare data for all samples in the list
prepared_data_list <- lapply(names(fora_results_h_lof_list), function(sample_id) {
  fora_results_h_lof_list[[sample_id]] %>%
    dplyr::mutate(Gene_ratio = overlap / size,
           Count = overlap,
           sample_id = sample_id) %>%
...
mm909_h_plot <- ggplot(combined_data, aes(x = sample_id, 
  ...
  labs(title = "FORA example with top H across samples",
       x = "Sample ID", 
       y = "Top H Gene Sets",
       color = "p.adjust",
       size = "Gene Ratio") +
...
# Save plot
ggsave(mm909_h_plot_path, mm909_h_plot, width = 12, height = 8, dpi = 300)
```
