...
sample_id_list <- as.list(unique(maf_df$sample_id))
```
...
# Connect to the Ensembl BioMart database for GRCh37
...
maf_df_annotated <- maf_df %>%
  ...
  dplyr::left_join(reannotated_df, by = "Entrez_Gene_Id") %>% 
  dplyr::distinct() %>% # Make sure only unique rows remain
  dplyr::group_by_at(vars(names(maf_df))) %>% # Group by everything except Ensembl Gene IDs
  dplyr::slice(1) %>% # Remove duplicated Ensemble Gene IDs, keep the first appearance
  dplyr::ungroup()
```

There is a "problem" here: some Entrez IDs are mapped to several Ensembl Gene IDs. I need to resolve this because otherwise it will impact the TMB calculation and possibly other analyses. I decided to keep the first Ensembl Gene ID that appears in the table for equal rows, but I need to double check if there is a better method to do this.

```{r}
# DEBUG: Rows that are equal but have 2 Ensembl_gene_ids
maf_df_annotated2 <- maf_df_annotated %>% 
  dplyr::select(-Ensembl_gene_id)
duplicated_rows <- maf_df_annotated2[duplicated(maf_df_annotated2) | duplicated(maf_df_annotated2, fromLast = TRUE), ]
duplicated_rows <- duplicated_rows %>%
  left_join(dplyr::select(maf_df_annotated, c(all_of(colnames(maf_df)), "Ensembl_gene_id")), by = colnames(maf_df)) %>% 
  dplyr::distinct()
duplicated_rows
```
...
lof_binary <- lof_vep_df %>%
  ...
  dplyr::select(sample_id, Ensembl_gene_id, SYMBOL, Liberal_consequence_binary) %>% 
  dplyr::distinct() %>% 
  dplyr::group_by(Ensembl_gene_id, SYMBOL) %>%
  pivot_wider(names_from = sample_id,
              values_from = Liberal_consequence_binary,
              values_fill = list(Liberal_consequence_binary = 0))  # Correctly fills missing values with 0
...
# Save results
## All mutations
fora_results_all_df <- data.frame(lapply(do.call(rbind, fora_results_all_list), set_lists_to_chars), stringsAsFactors = F)
write.csv(fora_results_all_df, 
          file = file.path(current_dir, 
                           "../../results/output/somatic_mutation_analysis/fora_results_all.csv"), 
          row.names = TRUE)
## Nonsyn mutations
fora_results_nonsyn_df <- data.frame(lapply(do.call(rbind, fora_results_nonsyn_list), set_lists_to_chars), stringsAsFactors = F)
write.csv(fora_results_nonsyn_df, 
          file = file.path(current_dir, 
                           "../../results/output/somatic_mutation_analysis/fora_results_nonsyn.csv"), 
          row.names = TRUE)
## LoF mutations
fora_results_lof_df <- data.frame(lapply(do.call(rbind, fora_results_lof_list), set_lists_to_chars), stringsAsFactors = F)
write.csv(fora_results_lof_df, 
          file = file.path(current_dir, 
                           "../../results/output/somatic_mutation_analysis/fora_results_lof.csv"), 
          row.names = TRUE)
```
...
```{r}
fora_results_lof_list$MM909_03 %>%
...
```{r}
# Define output path
mm909_03_immune_bp_plot_path <- file.path(current_dir,
                              "../../results/figures/mm909_03_immune_bp_plot.png")
...
# Save results
## All mutations
fora_results_h_all_df <- data.frame(lapply(do.call(rbind, fora_results_h_all_list), set_lists_to_chars), stringsAsFactors = F)
write.csv(fora_results_h_all_df, 
          file = file.path(current_dir, 
                           "../../results/output/somatic_mutation_analysis/fora_results_h_all.csv"), 
          row.names = TRUE)
## Nonsyn mutations
fora_results_h_nonsyn_df <- data.frame(lapply(do.call(rbind, fora_results_h_nonsyn_list), set_lists_to_chars), stringsAsFactors = F)
write.csv(fora_results_h_nonsyn_df, 
          file = file.path(current_dir, 
                           "../../results/output/somatic_mutation_analysis/fora_results_h_nonsyn.csv"), 
          row.names = TRUE)
## LoF mutations
fora_results_h_lof_df <- data.frame(lapply(do.call(rbind, fora_results_h_lof_list), set_lists_to_chars), stringsAsFactors = F)
write.csv(fora_results_h_lof_df, 
          file = file.path(current_dir, 
                           "../../results/output/somatic_mutation_analysis/fora_results_h_lof.csv"), 
          row.names = TRUE)
```
...
```{r}
# Define output path
mm909_03_h_apoptosis_plot_path <- file.path(current_dir,
                              "../../results/figures/mm909_03_h_apoptosis_plot.png")

# Filter the data
h_apoptosis <- fora_results_h_lof_list$MM909_03 %>%
...
# Save plot
ggsave(mm909_03_h_apoptosis_plot_path, h_apoptosis_plot, width = 10, height = 8, dpi = 300)
```
