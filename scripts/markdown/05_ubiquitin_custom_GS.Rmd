...
ub_db$ubiquitin_independent <- c(ub_db$ubiquitin_independent,
                                 `GO:0006355` = parent2child_list[["GO:0006355"]],
                                 `GO:0006508` = parent2child_list[["GO:0006508"]],
                                 `GO:0032446` = parent2child_list[["GO:0032446"]],
                                 `GO:0051726` = parent2child_list[["GO:0051726"]],
                                 `GO:0006325` = parent2child_list[["GO:0006325"]])
...
# Data.frame that maps go_parent_id to go_parent_term
go_parent_term_mapping2 <- ub_godb_bp_df %>% 
  dplyr::select(go_id, go_term) %>% 
  dplyr::filter(go_id %in% ub_parent_terms) %>% 
  dplyr::rename(go_parent_id = go_id,
                go_parent_term = go_term)
rownames(go_parent_term_mapping2) <- NULL

# Adding go_parent_term
annot_ub_glist_df2 <- annot_ub_glist_df2 %>%
  left_join(go_parent_term_mapping2, by = "go_parent_id")
...
# 645/674 genes remaining. Y genes lost in total. Here I used GRCh38 + GRCh37 + PFAM + Manual + More parents
...
So I defined some more parent terms for them and repeated the process. So far I’ve added 5 more GO parent terms. With that, I achieved that I’m only missing `r length(lost_genes5)` genes from the original list which still don’t have a parent GO term. So a total of 62 genes were rescued. 
