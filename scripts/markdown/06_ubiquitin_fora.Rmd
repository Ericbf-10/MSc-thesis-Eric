...
png(filename = venn_path, width = 1200, height = 800, res = 300)
...
mm909_ubiquitin_lof_plot_path <- file.path(figures_ubfora_path,
                              "ub_all_bubble_plot_RvsNR.png")

mm909_ubiquitin_lof_plot <- bubble_plot_all(response_df, fora_results_ubiquitin_lof_df, mm909_ubiquitin_lof_plot_path, "FORA with 5 Ubiquitin GS across samples (LoF mutations)", "Ubiquitin Gene Sets")
...
plot_path <- file.path(figures_ubfora_path, "ub_all_heat_map_RvsNR.pdf")
combinedGS_lof_plot <- complex_heatmap_genesets(response_df, fora_results_ubiquitin_lof_df, plot_path, 
                                    "Heat Map gene counts per Gene Set stratified by patient", 16, 6)
...
# Adapt structure to gene_sets_list
gene_sets_df_r <- r_annot_lof_df_ubiquitin %>% 
  dplyr::group_by(description) %>%
  dplyr::summarise(ensembl_ids = list(unique(ensembl_id)), .groups = 'drop')
gene_sets_df_nr <- nr_annot_lof_df_ubiquitin %>% 
  dplyr::group_by(description) %>%
  dplyr::summarise(ensembl_ids = list(unique(ensembl_id)), .groups = 'drop')

# Convert the dataframe to a named list
gene_sets_list_r <- setNames(gene_sets_df_r$ensembl_ids, gene_sets_df_r$description)
gene_sets_list_nr <- setNames(gene_sets_df_nr$ensembl_ids, gene_sets_df_nr$description)

# Perform FORA analysis for Ubiquitin Gene Sets
# perform_fora_analysis(sample_id_list, list_of_all_ub_mutations, list_of_nonsyn_ub_mutations, list_of_lof_ub_mutations, fora_results_all_csv_path_r, fora_results_nonsyn_csv_path_r, fora_results_lof_csv_path_r, gene_sets_list=gene_sets_list_r)
# perform_fora_analysis(sample_id_list, list_of_all_ub_mutations, list_of_nonsyn_ub_mutations, list_of_lof_ub_mutations, fora_results_all_csv_path_nr, fora_results_nonsyn_csv_path_nr, fora_results_lof_csv_path_nr, gene_sets_list=gene_sets_list_nr)
...
mm909_ubiquitin_lof_plot_r <- bubble_plot_pval(response_df, fora_results_ubiquitin_lof_df_r, mm909_ubiquitin_lof_plot_path_r, "FORA with 5 Ubiquitin GS across samples (LoF mutations) for Responders", "Ubiquitin Gene Sets")
...
mm909_ubiquitin_lof_plot_nr <- bubble_plot_pval(response_df, fora_results_ubiquitin_lof_df_nr, mm909_ubiquitin_lof_plot_path_nr, "FORA with 5 Ubiquitin GS across samples (LoF mutations) for Non-responders", "Ubiquitin Gene Sets")

print(mm909_ubiquitin_lof_plot_nr)
```

I don't see a clear pattern with the bubble plots. It seems that sample MM909_03 (NR) is an "outlier" compared to the other NR samples, since it has a lot of mutations in Ub genes.
...
## LoF mutations in unique Ubiquitin genes in R and NR -> annotation with Ub_db

```{r, warning=FALSE, message=FALSE}
ub_db_csv_path <- file.path(results_ubgenesets_path, "ub_db.csv")
...
saveRDS(joined_ub_info, file = patients_ub_lof_mut_path)
mock_df <- readRDS(patients_ub_lof_mut_path)

# Filter genes
r_final_ub_db_df <- joined_ub_info %>% 
  dplyr::filter(corrected_hugo_symbol %in% unique_responder_genes)
nr_final_ub_db_df <- joined_ub_info %>% 
  dplyr::filter(corrected_hugo_symbol %in% unique_non_responder_genes)
...
r_final_ub_db_df <- joined_ub_info %>% 
  dplyr::filter(corrected_hugo_symbol %in% unique_responder_genes) %>% 
  dplyr::select(ensembl_id, corrected_hugo_symbol, go_parent_term) %>% 
  dplyr::distinct()
...
shared_final_ub_db_df <- joined_ub_info %>% 
  dplyr::filter(corrected_hugo_symbol %in% shared_genes) %>% 
  dplyr::select(ensembl_id, corrected_hugo_symbol, go_parent_term) %>% 
  dplyr::distinct()
...
unique_shared_genes_path <- file.path(results_ubfora_path, 
                      "unique_and_shared_genes_RvsNR.rds")
saveRDS(combined_df, file = unique_shared_genes_path)

# Calculate the counts
grouped_counts <- combined_df %>%
  dplyr::select(ensembl_id, go_parent_term, source) %>% 
  group_by(go_parent_term, source) %>%
  summarise(count = n(), .groups = 'drop') %>% 
  complete(go_parent_term, source, fill = list(count = 0)) %>%
  arrange(go_parent_term, source)
...
# Create the bar plot
plot <- ggplot(grouped_counts, aes(x = go_parent_term, y = count, fill = source)) +
  ...
  labs(title = "Count of GO Parent Terms by Patient Response",
       x = "GO Parent Term",
       y = "Gene count",
...
plot_path <- file.path(figures_ubfora_path,
                              "barplot_R_NR_shared_go_terms.png")
ggsave(plot_path, plot, width = 14, height = 8, dpi = 300)
