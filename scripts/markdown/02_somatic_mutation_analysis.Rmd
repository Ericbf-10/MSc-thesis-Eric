...
maf_df <- maf_df %>%
  ...
  arrange(sample_id) %>% 
  mutate(Hugo_Symbol = replace(Hugo_Symbol, Hugo_Symbol == "Unknown", NA))
...
```{r}
maf_df_annotated %>%
  dplyr::group_by(sample_id, Codon_Change, Entrez_Gene_Id, Ensembl_gene_id) %>%
  dplyr::filter(n() > 1) %>% 
  dplyr::ungroup() %>% 
  dplyr::select(sample_id, Codon_Change, Entrez_Gene_Id, Ensembl_gene_id) %>% 
  dplyr::arrange(Entrez_Gene_Id)
```

```{r}
maf_df_annotated %>% 
  dplyr::filter(sample_id == "MM909_22" & Entrez_Gene_Id == 170679)
```
