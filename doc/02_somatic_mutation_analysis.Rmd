...
sequenced_region_size_mb <- 30  # For WES, it's commonly around 30 Mb.
...
datatable(tmb_df, 
          ...
          caption = "Number of mutations and TMB per patient"
        )
```
...
## Summary bar plot of LoF mutations

```{r}
# Define output path
lof_plot_path <- file.path(current_dir, 
                           "../results/lof_plot.png")

# Count the occurrences of each unique value in Consequence and calculate the true frequency
top_lof_variants <- lof_vep_df %>%
  count(Consequence, name = "frequency") %>%
  mutate(true_frequency = frequency / sum(frequency)) %>%
  arrange(desc(true_frequency)) %>%
  slice_max(order_by = true_frequency, n = 5)

# Create a bar plot with the true frequency
lof_plot <- ggplot(top_lof_variants, aes(x = reorder(Consequence, 
                                                     -true_frequency), 
                                         y = true_frequency, 
                                         fill = Consequence)) +
  geom_bar(stat = "identity") +
  ...
  labs(title = "Top 5 Most Frequent LoF Mutations (Relative Frequency)",
       x = "Consequence", 
       y = "Relative Frequency") +
  scale_y_continuous(breaks = seq(0, 1, by = 0.1))  # Set y-axis breaks every 0.1
...
# Save plot
ggsave(lof_plot_path, lof_plot, width = 10, height = 8, dpi = 300)
```
