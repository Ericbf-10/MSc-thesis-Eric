...
#### Get all GO.db ubiquitin terms (query=Ub genes).
#### Query GO database using mygene to find GO terms for Ub genes.
...
ub_categories <- c("ubiquitination", "deubiquitination", "ubiquitin_neutral", "ubiquitin_independent")
# ub_categories <- setNames(as.list(ub_categories), ub_categories)

# Parents (as character)
ub_parent_terms <- c("GO:0016567", "GO:0016579", "GO:0006511", "GO:0010499", "GO:0010992", "GO:0070086", "GO:0120323", "GO:0031397", "GO:0031398", "GO:2000059", "GO:2000060", "GO:0046872", "GO:0004842")
...
# Update ub_db
ub_db$ubiquitin_independent <- c(ub_db$ubiquitin_independent,
...
# Iterate only through the new elements in the ubiquitin_independent list
for (parent_id in new_ub_parent_terms) {
  ...
  for (child_id in children_ids) {
    ...
    if (!is.null(go_mapping2[[child_id]])) {
      go_mapping2[[child_id]] <- unique(c(go_mapping2[[child_id]], paste(parent_id, "ubiquitin_independent", sep = "|")))
    } else {
      go_mapping2[[child_id]] <- paste(parent_id, "ubiquitin_independent", sep = "|")
    }
  }
}
...
for (parent_id in new_ub_parent_terms2) {
  ...
  for (child_id in children_ids) {
    ...
    if (!is.null(go_mapping3[[child_id]])) {
      go_mapping3[[child_id]] <- unique(c(go_mapping3[[child_id]], paste(parent_id, "ubiquitin_independent", sep = "|")))
    } else {
      go_mapping3[[child_id]] <- paste(parent_id, "ubiquitin_independent", sep = "|")
    }
  }
}
...
# Join the mapping information back to the lost_genes4 data.frame. The logic here is that I map the new parents only to those genes where I could not find a ubiquitin-related GO term.
final_df3 <- final_df_filtered_all_na %>%
...
# Amend cases where child belongs to 2 parents
annot_ub_glist_df3 <- annot_ub_glist_df3 %>%
  dplyr::mutate(
    # Check if 'go_parent_id' contains multiple values and update accordingly
    go_parent_id = ifelse(grepl(",", go_parent_id) & grepl("GO:0051726", go_parent_id),
                          "GO:0016579",
                          ifelse(grepl(",", go_parent_id) & grepl("GO:0006508", go_parent_id),
                                 "GO:0006508",
                                  go_parent_id))
  )
...
```{r, warning=FALSE, message=FALSE}
# map_category2parent_df <- final_ub_db %>%
...
#10 Perform FORA analysis
# Define paths
fora_results_all_csv_path2 <- file.path(results_ubgenesets_path, "fora_results_ubiquitin_cat_all.csv")
fora_results_nonsyn_csv_path2 <- file.path(results_ubgenesets_path, "fora_results_ubiquitin_cat_nonsyn.csv")
fora_results_lof_csv_path2 <- file.path(results_ubgenesets_path, "fora_results_ubiquitin_cat_lof.csv")
...
```{r, message=FALSE, warning=FALSE, fig.width=10, fig.height=6}
# Define output path
mm909_ubiquitin_all_plot_path2 <- file.path(figures_ubgenesets_path,
...
```{r, message=FALSE, warning=FALSE, fig.width=10, fig.height=6}
# Define output path
mm909_ubiquitin_nonsyn_plot_path2 <- file.path(figures_ubgenesets_path,
...
```{r, message=FALSE, warning=FALSE, fig.width=10, fig.height=6}
# Define output path
mm909_ubiquitin_lof_plot_path2 <- file.path(figures_ubgenesets_path,
                              "mm909_ubiquitin_cat_lof_plot.png")

mm909_ubiquitin_lof_plot <- plot_top5(response_df, fora_results_ubiquitin_lof_df2, mm909_ubiquitin_lof_plot_path2, "FORA with top Custom Ubiquitin GS across samples (LoF mutations)", "Top Ubiquitin Gene Sets")
...
```{r, warning=FALSE, message=FALSE, fig.width=10, fig.height=10}
plot_path <- file.path(figures_ubgenesets_path, "mm909_combinedGS_cat_lof_heat_map.png")
