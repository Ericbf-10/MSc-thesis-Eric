...
# Merge the dub_genes and C19_genes
## Filter C19_genes that are not already in dub_genes
new_genes <- corrected_hsymbols_updated %>%
  dplyr::filter(category == "C19_genes") %>%
  dplyr::anti_join(corrected_hsymbols_updated %>%
              dplyr::filter(category == "dub_genes"),
            by = "corrected_hugo_symbol")

## Update the category to dub_genes for these new genes
new_genes <- new_genes %>%
  dplyr::mutate(category = "dub_genes", description = "Deubiquitinases")

## Add these new genes to the original data frame
ub_classes_final <- corrected_hsymbols_updated %>%
  dplyr::filter(category != "C19_genes") %>%
  bind_rows(new_genes)

# Extract the ensembl_id column to do the filtering
ubiquitin_df <- ub_classes_final %>% 
  dplyr::select(ensembl_id) %>% 
  dplyr::distinct()

# Get ubiquitin unique ensembl ids
ub_ensembl_ids <- unique(ub_classes_final$ensembl_id)
...
# Define output path
mm909_sum_plot_path <- file.path(figures_ubfora_path,
                              "mm909_sum_plot_ubi_nr.png")

# Summary plot
mm909_sum_plot <- sum_plot(response_df, all_df_ubiquitin, nonsyn_df_ubiquitin, lof_df_ubiquitin, mm909_sum_plot_path, select_r = FALSE)

# Display
print(mm909_sum_plot)
```

Sample MM909_47 does not have any Ubiquitin-related gene. There are 130 ubiquitin LoF mutations in total.

# Define Gene Sets based on input file

```{r, warning=FALSE, message=FALSE}
# Count how many genes each category has initially
raw_category_gene_count <- ub_classes_final %>% 
...
cat_lof_df_ubiquitin <- lof_df_ubiquitin %>% 
  dplyr::left_join(ub_classes_final, by="ensembl_id")
...
```{r, warning=FALSE, message=FALSE}
ub_db_path <- file.path(results_ubi_db_path, "ub_db.rds")
final_ub_db_df <- readRDS(ub_db_path)
...
plot <- barplot_mean_sd(final_go_df, "go_parent_term", "GO Term", "Mean gene counts", 13, 6, -14.5, 0.5, 3, plot_path)
print(plot)
```

I don't see all 40 go parent terms because not all of them are present in the lof ubi mutations.

This bar plot shows genes in R and NR However, a gene may participate in different biological processes. 
